# 00 产品 PRD：Local-first Personal Agent OS

## 1. 产品定位

本产品是一个运行在用户自有 Mac / Linux 主控节点上的 Local-first Personal Agent OS。它通过 Web、Telegram、后续微信和 CLI 等入口，统一调度 Agent、模型、工具、长期记忆和节点资源。
...
5. 可以通过前端 UI 查看对话、执行过程、记忆、节点、工具和模型路由。
6. 不希望所有消息被一个严格模型中转。
7. 不希望模型直接乱调工具。
...
模型只请求 capability，例如 `server_diagnose`。系统把它编译为固定 workflow，例如 docker 状态、日志、端口、资源检查。模型不能直接执行 shell，毕竟把生产环境交给概率模型相当于把方向盘递给喝多的导航仪。
...
没有 UI，用户只能相信黑箱。Agent OS Console 必须展示 Chat、Run Trace、Memory Studio、Agent Registry、Node Console、Capability Console 和 Model Routing。

## 4. 核心模块

| 模块 | 说明 |
|---|---|
| Gateway | Web / Telegram / 后续微信入口 |
| Orchestrator Core | 控制平面核心 |
...
| Tool Compiler | Capability 转 Tool Workflow |
| Policy Engine | 权限、安全、隐私、确认机制 |
| Node Scheduler | 手动/自动节点选择 |
| Worker Runtime | 节点执行任务 |
| Run Trace | 全链路追踪 |
| Web Console | 前端控制台 |
...
1. Web Chat 普通问答。
2. Web Chat 触发 Memory Search。
3. devops_agent 触发 server_diagnose capability。
4. 手动指定 Worker 执行 web_research 或只读任务。
